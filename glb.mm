            ...
            if(mimeType) {
                ...
                if(dict) {
                    ...
                    for(int n=1; n<v.size()/3; n++) {
                        ...
                        v_min[0] = std::min(v_min[0],v[n*3+0]);
                        v_min[1] = std::min(v_min[1],v[n*3+1]);
                        v_min[2] = std::min(v_min[2],v[n*3+2]);
                        
                        v_max[0] = std::max(v_max[0],v[n*3+0]);
                        v_max[1] = std::max(v_max[1],v[n*3+1]);
                        v_max[2] = std::max(v_max[2],v[n*3+2]);
                    }
